...
Node *build_tree(uint64_t hist
    ...
    for (int i = 0; i < ALPHABET;
         i += 1) { //min heap = every parent node <= the value of the children
        if (hist[i] > 0) {
...
void dump_tree(int outfile, Node *root) {
    ...
    if (root != NULL) {
        ...
        if (root->left == NULL && root->right == NULL) {
            ...
            write_bytes(
                outfile, &leaf, 1); //when on a leaf node "L" followed by the symbol of node "c"
            write_bytes(outfile, &root->symbol, 1);
        } else { //if this is not a interior
            //	tree_buffer[1] = 'I'; //setting the 'I'
            write_bytes(outfile, &interior,
                1); //if it is an interior node "I" followed by the symbol of the node
        }
    }
    return;
}
...
Node *rebuild_tree(uint16_t nbytes,
    uint8_t tree[static nbytes]) { //CITE: TA Eugene for rebuild tree sudo in 11/4 section
    ...
    for (uint32_t i = 0; i < nbytes; i += 1) { //if we dump 'L'
        ...
        if (tree[i] == 'I') { //if we dump 'I'
            ...
            Node *n = node_join(
                left, right); // the parent is n and will be using node join for left and right
