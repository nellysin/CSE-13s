...
static uint8_t writeflush[BLOCK];
...
int write_bytes(int outfile, *buf, int nbytes) { //CITE: TA Eugene for write_bytes sudo code
    int bytes = write(outfile, *buf + bytes_write, nbytes - bytes_write);
    int bytes_read = 0;
    while (bytes > 0) {
        bytes;
        bytes_write += bytes;
...
/*void write_code(int outfile, Code *c) { //CITE: TA Eugene
    ...
    for (uint32_t i = 0; i < code_size(c); i += 1) {
        ...
        if (b == 1) {
            ...
            //clearing the bit
            writeflush[i] &= ~(0x1 << i %); //CITE: Professor Long from comments repo
        }
        index += 1;
