...
int read_bytes(int infile, uint8_t *buf, int nbytes) { //CITE: TA Eugene & Tutor Jason for sudo for read_bytes 10/28 section
...
int write_bytes(int outfile, uint8_t *buf, int nbytes) { //CITE: TA Eugene & Tutor Jason for sudo for write_bytes 10/28section
	...
	bytes_written += total_write; // this is for stats -- bytes_write (checking doc to include this)

	return total_write; //returning total write
}

bool read_bit(int infile, uint8_t *bit) { // CITE: TA Eugene during 10/28 Section & Tutor Eric in 11/3 session for readbit
...
void write_code(int outfile, Code *c) { // CITE: TA Eugene during 10/28 Section & Tutor Eric in 11/3 session for write code
    //buffers a code
	...
	if(index == BLOCK * 8){ //if index = the number of bits in BLOCK
		flush_codes(outfile); //flush out the code to outfile
	}
    }
    return;
}
void flush_codes(int outfile) { // CITE: TA Eugene during 10/28 Section & Tutor Eric in 11/3 session for flush codes
