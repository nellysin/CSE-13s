...
void rsa_make_pub(mpz_t p, mpz_t q, mpz_t n, mpz_t e, uint64_t nbits, uint64_t iters) {
    mpz_t p_min, q_min, gcd_totient;
    mpz_inits(p_min, q_min, gcd_totient, NULL); 
    ...
    do {
       ...
       gcd(gcd_totient, e, n); //the totient
    } while (mpz_cmp_ui(gcd_totient, 1));
    mpz_clears(p_min, q_min, gcd_totient, NULL);//clearing the memory
}
