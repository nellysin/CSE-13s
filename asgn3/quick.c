...
uint64_t partition(
    Stats *stats, uint32_t *A, uint64_t lo, uint64_t hi) { // this is partition helper function
...
void quick_sort(
    Stats *stats, uint32_t *A, uint32_t n) { //the actual qucik sort that calls the helper functions
